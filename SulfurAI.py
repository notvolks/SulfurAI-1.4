...
def retrieve_app_info():
    """
    Reads from a file and returns a list with each line as an item.


    Returns:
        list: List containing each line from the file as an item (including empty lines)
    """
    file_path = call.app_info()
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            # Read all lines and strip only the newline character
